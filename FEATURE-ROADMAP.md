...
| FR-014 | Add split-pane and multi-session workspace layouts with keyboard navigation, pane resizing, clear active-pane state, and continuity-safe restoration. | High | In progress: bounded split metadata and runtime launch planning exist with deterministic pane order and one initial active pane; GTK split rendering, keyboard pane navigation/resizing, and continuity restoration remain open |
...
1. complete live profile selection and workspace activation using the bounded profile/workspace runtime plans, then render multi-pane GTK layouts with explicit active-pane state, keyboard navigation, and resizing;
...
3. implement SSH/remote lifecycle, host verification, reconnect behavior, and trustworthy remote context-state presentation;
