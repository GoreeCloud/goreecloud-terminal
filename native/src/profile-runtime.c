...
static void
set_catalog_error(GError **error, const char *message)
{
...
{
    if (catalog == NULL || catalog->profiles == NULL || profile_id == NULL)
        return NULL;
    ...
    GError **error)
{
    if (catalog == NULL || catalog->profiles == NULL ||
        ...
        set_catalog_error(error, "Terminal runtime catalog is incomplete.");
        return FALSE;
    }
    ...
    for (guint index = 0; index < catalog->profiles->len; index++) {
        ...
        if (g_hash_table_contains(profile_ids, profile->id)) {
            set_catalog_error(error, "Terminal runtime catalog contains duplicate profile IDs.");
            g_hash_table_unref(profile_ids);
            return FALSE;
        }
    ...
    for (guint index = 0; index < catalog->workspaces->len; index++) {
        ...
        if (g_hash_table_contains(workspace_ids, workspace->id)) {
            set_catalog_error(error, "Terminal runtime catalog contains duplicate workspace IDs.");
            g_hash_table_unref(profile_ids);
            g_hash_table_unref(workspace_ids);
            return FALSE;
        }
    ...
    GoreeTerminalHostLaunchContext *context)
{
    ...
    context->shell_path = profile->shell_path != NULL ? profile->shell_path : "";
    context->working_directory = profile->working_directory != NULL
        ? profile->working_directory
        : "";
    context->environment_policy = profile->environment_policy == GOREE_TERMINAL_ENVIRONMENT_CLEAN
        ? GOREE_TERMINAL_HOST_ENVIRONMENT_CLEAN
        : GOREE_TERMINAL_HOST_ENVIRONMENT_INHERIT_SAFE;
    context->environment_names = (const char *const *) profile->environment_allowlist;
    context->environment_count = profile->environment_allowlist != NULL
